...
function Dropdown({ item, isOpen, onClose }: { item: NavItem; isOpen: boolean; onClose: () => void }) {
  ...
  return (
    ...
    >
      {/* Seta decorativa */}
      <div
        className="absolute top-[6px] left-1/2 -translate-x-1/2 w-2.5 h-2.5 bg-white border-l border-t border-slate-200 rotate-45 rounded-tl-xs"
        style={{ zIndex: 1 }}
      />

      {/* Painel */}
      <div className="relative bg-white rounded-xl border border-slate-200 shadow-lg p-1.5">
        {item.children.map((sub) => (
          <Link key={sub.href} to={sub.href} onClick={onClose} className="block no-underline group">
            <div className="flex items-start gap-3 px-3 py-2 rounded-lg hover:bg-slate-50 transition-colors">
              <div className="w-7 h-7 rounded-md bg-slate-100 border border-slate-200/60 flex items-center justify-center flex-shrink-0 text-slate-600 group-hover:bg-blue-50 group-hover:text-blue-700 group-hover:border-blue-200 transition-all mt-0.5">
                {sub.icon}
              </div>
              <div>
                <div className="text-[13px] font-semibold text-slate-800 leading-tight mb-0.5">
                  {sub.name}
                </div>
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <nav className={`
        ...
        ${scrolled ? "bg-white/95 backdrop-blur-md border-b border-slate-200/90 shadow-xs" : "bg-white/80 backdrop-blur-sm border-b border-slate-100"}
      `}>
        <div className="max-w-[1120px] mx-auto px-6 h-16 flex items-center justify-between">

          {/* LOGO CÍVICO */}
          <Link to="/" className="flex items-center gap-2.5 no-underline group">
            <div className="w-8 h-8 bg-slate-900 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-200 group-hover:bg-blue-700 shadow-xs">
              <Icons.Logo />
            </div>
            <span style={{ fontFamily: "'DM Mono', monospace" }} className="text-base font-semibold text-slate-900 tracking-tight">
                    ...
                    <button
                      ...
                      className={`
                        ...
                        ${isOpen
                          ...
                          : "bg-slate-900 text-white border-slate-900 hover:bg-blue-700 hover:border-blue-700 shadow-xs"
                        }
                      `}
                      style={{ fontFamily: "'DM Sans', sans-serif" }}
                    >
                      Explorar dados
                      <span className={`opacity-80 transition-transform duration-200 flex ${isOpen ? "rotate-180" : ""}`}>
                        <Icons.ChevronDown />
                      </span>
                    </button>
