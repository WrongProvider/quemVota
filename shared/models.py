...
eventosOrgaos = Table(
    ...
    Column(
        "idOrgao",
        Integer,
        ForeignKey("orgaos.id", ondelete="CASCADE"),
        primary_key=True,
    ),
)
...
proposicoesTemas = Table(
    ...
    Column(
        "idTema", Integer, ForeignKey("temas.id", ondelete="CASCADE"), primary_key=True
    ),
)
...
class LegislaturaMesa(Base):
    ...
    idLegislatura = Column(
        Integer,
        ForeignKey("legislaturas.idLegislatura", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    idDeputado = Column(
        Integer,
        ForeignKey("deputados.idCamara", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "cargo",
            "dataInicio",
            name="uq_mesa_leg_dep_cargo_data",
        ),
    )
...
class OrgaoDeputado(Base):
    ...
    id = Column(Integer, primary_key=True)
    idOrgao = Column(
        Integer, ForeignKey("orgaos.id", ondelete="CASCADE"), nullable=False, index=True
    )
    idDeputado = Column(
        Integer,
...
class Deputado(Base):
    ...
    )  # Gerado a partir do nome para URLs amigáveis
    # Identificação
    ...
    # Legislaturas
    idLegislaturaInicial = Column(
        Integer, ForeignKey("legislaturas.idLegislatura"), nullable=True
    )
    idLegislaturaFinal = Column(
        Integer, ForeignKey("legislaturas.idLegislatura"), nullable=True, index=True
    )
    ...
    idPartido = Column(
        Integer, ForeignKey("partidos.id", ondelete="SET NULL"), nullable=True
    )
    ...
    legislatura = relationship(
        "Legislatura", back_populates="deputados", foreign_keys=[idLegislaturaFinal]
    )
    ...
    ocupacoes = relationship(
        "DeputadoOcupacao", back_populates="deputado", cascade="all, delete-orphan"
    )
    profissoes = relationship(
        "DeputadoProfissao", back_populates="deputado", cascade="all, delete-orphan"
    )
    despesas = relationship(
        "Despesa", back_populates="deputado", cascade="all, delete-orphan"
    )
    discursos = relationship(
        "Discurso", back_populates="deputado", cascade="all, delete-orphan"
    )
    votos = relationship(
        "Voto", back_populates="deputado", cascade="all, delete-orphan"
    )
    ...
    buscaPopular = relationship(
        "BuscaPopular", back_populates="deputado", uselist=False
    )
...
class Evento(Base):
    ...
    requerimentos = relationship(
        "EventoRequerimento", back_populates="evento", cascade="all, delete-orphan"
    )
    presencas = relationship(
        "PresencaDeputado", back_populates="evento", cascade="all, delete-orphan"
    )
...
class Proposicao(Base):
    ...
    autores = relationship(
        "ProposicaoAutor", back_populates="proposicao", cascade="all, delete-orphan"
    )
    temas = relationship(
        "Tema", secondary="proposicoesTemas", back_populates="proposicoes"
    )
    tramitacoes = relationship(
        "Tramitacao",
        back_populates="proposicao",
        cascade="all, delete-orphan",
        order_by="Tramitacao.dataHora",
    )
...
class Tema(Base):
    ...
    proposicoes = relationship(
        "Proposicao", secondary="proposicoesTemas", back_populates="temas"
    )
...
class Votacao(Base):
    ...
    orientacoes = relationship(
        "VotacaoOrientacao", back_populates="votacao", cascade="all, delete-orphan"
    )
    objetos = relationship(
        "VotacaoObjeto", back_populates="votacao", cascade="all, delete-orphan"
    )
...
class Frente(Base):
    ...
    coordenador_id = Column(
        Integer, ForeignKey("deputados.id", ondelete="SET NULL"), nullable=True
    )
    ...
    membros = relationship(
        "FrenteDeputado", back_populates="frente", cascade="all, delete-orphan"
    )
...
class FrenteDeputado(Base):
    ...
    idFrente = Column(
        Integer,
        ForeignKey("frentes.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    idDeputado = Column(
...
class Grupo(Base):
    ...
    historico = relationship(
        "GrupoHistorico", back_populates="grupo", cascade="all, delete-orphan"
    )
...
class GrupoMembro(Base):
    ...
    id = Column(Integer, primary_key=True)
    idGrupo = Column(
        Integer, ForeignKey("grupos.id", ondelete="CASCADE"), nullable=False, index=True
    )
    idDeputado = Column(
        Integer,
        ForeignKey("deputados.id", ondelete="CASCADE"),
...
class Licitacao(Base):
    ...
    pedidos = relationship(
        "LicitacaoPedido", back_populates="licitacao", cascade="all, delete-orphan"
    )
    propostas = relationship(
        "LicitacaoProposta", back_populates="licitacao", cascade="all, delete-orphan"
    )
    itens = relationship(
        "LicitacaoItem", back_populates="licitacao", cascade="all, delete-orphan"
    )
    contratos = relationship(
        "LicitacaoContrato", back_populates="licitacao", cascade="all, delete-orphan"
    )
...
class LicitacaoPedido(Base):
    ...
    idLicitacao = Column(
        Integer,
        ForeignKey("licitacoes.idLicitacao", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ano = Column(Integer, index=True)
...
class BuscaPopular(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    idDeputado = Column(
        Integer, ForeignKey("deputados.id"), nullable=False, unique=True
    )
