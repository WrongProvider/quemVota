...
from injest_banco.schemas_camara import parse_flexible_datetime
...
def upsert_proposicao(db: SessionLocal, d: dict) -> Proposicao:
    ...
    prop.data_apresentacao = parse_flexible_datetime(d.get("dataApresentacao"))
...
def rodar_backfill_votacoes(limit: int | None = None, workers: int = DEFAULT_WORKERS):
    with SessionLocal() as db:
        ...
        logger.info(
            f"🚀 Iniciando Backfill: {total} votações sem proposição ({workers} workers)."
        )
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if not uri_prop:
                        ...
                        logger.debug(
                            f"[{processadas}/{total}] ℹ️ Votação {id_votacao} é administrativa (sem proposição)."
                        )
                        continue
                    ...
                    if id_prop_camara is None:
                        logger.warning(
                            f"[{processadas}/{total}] URI de proposição inesperada na votação {id_votacao}: {uri_prop}"
                        )
                        erros += 1
                        continue
                    ...
                    if prop is None:
                        prop = (
                            db.query(Proposicao)
                            .filter_by(id_camara=id_prop_camara)
                            .first()
                        )

                    if prop is None:
                        logger.info(
                            f"[{processadas}/{total}] ⚡ Proposição {id_prop_camara} em falta. Baixando..."
                        )
                        prop_payload = camara_get(f"/proposicoes/{id_prop_camara}").get(
                            "dados", {}
                        )
                        if prop_payload:
                    ...
                    if prop is not None:
                        ...
                        vinculadas += 1
                        logger.info(
                            f"[{processadas}/{total}] 🔗 Votação {id_votacao} associada com sucesso à Proposição {id_prop_camara}."
                        )
                    else:
                        erros += 1
                        logger.warning(
                            f"[{processadas}/{total}] Proposição {id_prop_camara} não retornou dados da API."
                        )
                    ...
                    erros += 1
                    logger.error(
                        f"[{processadas}/{total}] ❌ Erro na votação {id_votacao}: {e}"
                    )

                if processadas % LOG_PROGRESS_EVERY == 0:
                    logger.info(
                        f"Progresso: {processadas}/{total} | {vinculadas} vinculadas | {erros} erros"
                    )

    logger.info("🏁 Backfill Concluído!")
    logger.info(
        f"📊 Resumo: {vinculadas} vinculadas | {sem_proposicao_na_origem} eram administrativas | {erros} erros"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Backfill de votações órfãs (sem proposição vinculada)."
    )
    parser.add_argument(
        "--limit",
        type=int,
