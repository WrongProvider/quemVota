...
def inicializar_modelo_ia():
    ...
    if not os.path.exists(MODELO_IA_CAMINHO):
        raise FileNotFoundError(
            f"Arquivo '{MODELO_IA_CAMINHO}' não encontrado. Baixe-o e coloque na pasta do script."
        )
...
def baixar_e_converter_fotos():
    ...
    with SessionLocal() as db:
        ...
        politicos = (
            db.query(Deputado.id, Deputado.urlFoto)
            .filter(Deputado.urlFoto.isnot(None))
            .all()
        )
        ...
        for politico_id, url in politicos:
            ...
            try:
                ...
                resposta = requests.get(
                    url, headers=headers, timeout=TIMEOUT_REQUISICAO
                )
                ...
                if img_cv2 is None:
                    print(
                        f"[{politico_id}] ERRO: A imagem baixada está corrompida ou é inválida."
                    )
                    continue
